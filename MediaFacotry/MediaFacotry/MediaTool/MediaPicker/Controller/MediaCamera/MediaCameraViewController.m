 ...
 -  (void)setupUI
{
    ...
    self.bottomView.backgroundColor = [MediaColor(244, 244, 244) colorWithAlphaComponent:.9];
    ...
    self.cancelBtn.backgroundColor = [MediaColor(244, 244, 244) colorWithAlphaComponent:.9];
    ...
    [self.doneBtn setImage:[UIImage imageNamed:@"takeok"] forState:UIControlStateNormal];
 ...
 -  (void)showCancelDoneBtn
{
    ...
    doneRect.origin.x = self.width  -  doneRect.size.width  -  40;
...
@interface MediaCameraViewController ()<CameraToolViewDelegate, AVCaptureFileOutputRecordingDelegate>
{
    //拖拽手势开始的录制
    BOOL _dragStart;
    BOOL _layoutOK;
...
@implementation MediaCameraViewController
 ...
 -  (void)viewDidLayoutSubviews
{
    ...
    self.toolView.frame = CGRectMake(0, kMediaScreenHeight - 130 - Media_SafeAreaBottom, kMediaScreenWidth, 100);
    self.previewLayer.frame = self.view.layer.bounds;
    self.toggleCameraBtn.frame = CGRectMake(kMediaScreenWidth - 50, 20, 30, 30);
}
 ...
 -  (void)setupUI
{
    ...
    self.focusCursorImageView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"focus"]];
    ...
    [self.toggleCameraBtn setImage:[UIImage imageNamed:@"toggle_camera"] forState:UIControlStateNormal];
 ...
 -  (NSString *)transformSessionPreset
{
    ...
    if (@available(iOS 9.0, *)) {
        if (MediaCaptureSessionPreset3840x2160) {
 ...
 -  (NSURL *)getVideoFileUrl
{
    ...
    NSString *format = self.videoType == MediaVideoExportTypeMOV ? @"mov" : self.videoType == MediaVideoExportTypeMP4 ? @"mp4" : @"3gp";
