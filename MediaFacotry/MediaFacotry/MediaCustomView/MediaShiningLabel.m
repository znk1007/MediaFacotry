...
@implementation MediaShiningLabel
- (instancetype)init {
    if (self = [super init]) {
...
- (void)myInit {
    ...
    self.shimmerType = MediaLeftToRight;
    self.repeat = true;
...
- (void)freshMaskLayer {
    if (self.shimmerType != MediaShimmerAll) {
        _maskLayer.backgroundColor = [UIColor clearColor].CGColor;
...
- (CABasicAnimation *)translate {
    ...
    _translate.autoreverses = self.shimmerType == MediaAutoReverse ? true : false;
