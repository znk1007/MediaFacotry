...
NSString *const MediaCSVErrorDomain = @"com.mediacsvparse.csv";
...
#pragma mark - 、、、、、、、、、、、转换工具类、、、、、、、、、、、、、、、
@interface MediaCSVAggregator : NSObject <MediaCSVParserDelegate>
...
@implementation MediaCSVAggregator
...
@interface MediaCSVKeyedAggregator : MediaCSVAggregator

/**
 首行
 */
@property (nonatomic, strong) NSArray *firstLine;
@end

@implementation MediaCSVKeyedAggregator

- (void)parser:(MediaCSVParser *)parser didEndLine:(NSUInteger)recordNumber {
    if (self.firstLine == nil) {
        self.firstLine = self.currentLine;
    } else if (self.currentLine.count == self.firstLine.count) {
        MediaCSVOrderedDictionary *line = [[MediaCSVOrderedDictionary alloc] initWithObjects:self.currentLine
                                                                               forKeys:self.firstLine];
        [self.lines addObject:line];
    } else {
        [parser cancelParsing];
        self.error = [NSError errorWithDomain:MediaCSVErrorDomain code:MediaCSVErrorCodeIncorrectNumberOfFields userInfo:nil];
    }
    self.currentLine = nil;
}
@end

NSArray *MediaCSVParserParse(NSInputStream *inputStream, MediaCSVParserOptions options, unichar delimiter, NSError *__autoreleasing *error);
NSArray *MediaCSVParserParse(NSInputStream *inputStream, MediaCSVParserOptions options, unichar delimiter, NSError *__autoreleasing *error) {
    MediaCSVParser *parser = [[MediaCSVParser alloc] initWithInputStream:inputStream usedEncoding:nil delimiter:delimiter];
    
    BOOL usesFirstLineAsKeys = !!(options & MediaCSVParserOptionsUsesFirstLineAsKeys);
    MediaCSVAggregator *aggregator = usesFirstLineAsKeys ? [[MediaCSVKeyedAggregator alloc] init] : [[MediaCSVAggregator alloc] init];
    parser.delegate = aggregator;
    
    parser.recognizesBackslashesAsEscapes = !!(options & MediaCSVParserOptionsRecognizesBackslashesAsEscapes);
    parser.sanitizesFields = !!(options & MediaCSVParserOptionsSanitizesFields);
    parser.recognizesComments = !!(options & MediaCSVParserOptionsRecognizesComments);
    parser.trimsWhitespace = !!(options & MediaCSVParserOptionsTrimsWhitespace);
    parser.recognizesLeadingEqualSign = !!(options & MediaCSVParserOptionsRecognizesLeadingEqualSign);
...
@implementation NSArray (MediaCSVAdditions)
...
+ (instancetype)arrayWithContentsOfDelimitedURL:(NSURL *)fileURL options:(MediaCSVParserOptions)options delimiter:(unichar)delimiter error:(NSError *__autoreleasing *)error {
    ...
    return MediaCSVParserParse(stream, options, delimiter, error);
}

- (NSString *)CSVString {
    NSOutputStream *output = [NSOutputStream outputStreamToMemory];
    MediaCSVWriter *writer = [[MediaCSVWriter alloc] initWithOutputStream:output encoding:NSUTF8StringEncoding delimiter:COMMA];
...
@implementation NSString (MediaCSVAdditions)
...
@implementation MediaCSVOrderedDictionary {
    NSArray *_keys;
    NSArray *_values;
...
- (BOOL)isEqual:(MediaCSVOrderedDictionary *)object {
    if ([super isEqual:object] && [object isKindOfClass:[MediaCSVOrderedDictionary class]]) {
...
@interface MediaCSVParser ()
@property (assign) NSUInteger totalBytesRead;
@end

@implementation MediaCSVParser {
...
- (BOOL)_parseDelimiter {
    ...
    if (next != NULLCHAR && [[NSCharacterSet newlineCharacterSet] characterIsMember:next] == NO) {
        ...
        _error = [[NSError alloc] initWithDomain:MediaCSVErrorDomain code:MediaCSVErrorCodeInvalidFormat userInfo:@{NSLocalizedDescriptionKey : description}];
    }
    return NO;
}
