...
#define APP_ID @"1253373558"

#import "MediaVersionManager.h"
#import <StoreKit/StoreKit.h>
@interface MediaVersionManager ()<SKStoreProductViewControllerDelegate>
...
@implementation MediaVersionManager
static MediaVersionManager *checkManager = nil;
+ (instancetype)sharedCheckManager {
    
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        checkManager = [[MediaVersionManager alloc] init];
...
- (void)compareWithCurrentVersion {
    ...
    if (![[userDefault objectForKey:APP_LAST_VERSION] isEqualToString:CURRENT_VERSION]) {
        if (@available(iOS 8.0, *)) {
            ...
            if (self.confimTitle != nil && ![self.confimTitle isEqualToString:@""]) {
            ...
            if (self.nextTimeTitle != nil && ![self.nextTimeTitle isEqualToString:@""]) {
            ...
            if (self.skipVersionTitle != nil && ![self.skipVersionTitle isEqualToString:@""]) {
