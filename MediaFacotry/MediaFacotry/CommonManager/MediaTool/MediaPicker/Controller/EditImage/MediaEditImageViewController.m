//
//  MediaEditImageViewController.m
//  MediaFacotry
//
...
- (UIImageView *)iconView{
    if (!_iconView) {
        _iconView = [[UIImageView alloc] initWithFrame:CGRectMake(10, 5, CGRectGetWidth(self.frame)-20, CGRectGetWidth(self.frame)-20)];
...
- (UILabel *)titleLabel{
    if (!_titleLabel) {
        _titleLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, CGRectGetMaxY(self.iconView.frame) + 5, CGRectGetWidth(self.frame), 15)];
        _titleLabel.backgroundColor = [UIColor clearColor];
        _titleLabel.textColor = MediaColor(18, 18, 18);
...
- (void)refreshViews
{
    self.titleLabel.text = [_ratio description];
...
@interface MediaEditImageViewController ()
{
...
@implementation MediaEditImageViewController
...
- (void)viewDidLayoutSubviews
{
    ...
    CGFloat w = kMediaScreenWidth-20;
    CGFloat maxH = kMediaScreenHeight-flag-inset.bottom-inset.top-50;
    ...
    _imageView.frame = CGRectMake((kMediaScreenWidth-w)/2, (kMediaScreenHeight-flag-h)/2, w, h);
    ...
    _bottomView.frame = CGRectMake(0, kMediaScreenHeight-bottomViewH-inset.bottom, kMediaScreenWidth, bottomViewH);
    _cancelBtn.frame = CGRectMake(10+inset.left, 7, 60, bottomBtnH);
    _saveBtn.frame = CGRectMake(kMediaScreenWidth/2-20, 7, 40, bottomBtnH);
    ...
    if (hideClipRatioView) {
        ...
        _rotateBtn.superview.frame = CGRectMake(kMediaScreenWidth-70-inset.right, kMediaScreenHeight-128-inset.bottom, 70, 80);
        _menuScroll.frame = CGRectMake(inset.left, kMediaScreenHeight-128-inset.bottom, kMediaScreenWidth-70-inset.left-inset.right, 80);
    }
}
...
- (void)loadImage
{
    ...
    CGSize size = CGSizeMake(width*scale, width*scale*self.model.phAsset.pixelHeight/self.model.phAsset.pixelWidth);
    
    [_indicator startAnimating];
    __weak typeof(self) weakSelf = self;
    [[MediaFactory sharedFactory].photo requestImageForAsset:self.model.phAsset size:size completion:^(UIImage *image, NSDictionary *info) {
...
- (void)setCropMenu
{
    ...
    _menuScroll = [[UIScrollView alloc] initWithFrame:CGRectMake(0, 0, kMediaScreenWidth-70, 80)];
...
- (void)setSelectedMenu:(MediaRatioMenuItem *)selectedMenu
{
    if(selectedMenu != _selectedMenu){
        ...
        _selectedMenu.backgroundColor = MediaColor(30, 30, 30);
...
- (void)saveBtn_click
{
    ...
    [hud show];
    [[MediaFactory sharedFactory].photo saveToAlbumWithImage:[self clipImage] completion:^(BOOL suc, PHAsset *asset) {
        [hud hide];
        if (!suc) {
...
{
    //确定裁剪，返回
    if ([MediaFactory sharedFactory].tool.callSelectClipImageBlock) {
        [MediaFactory sharedFactory].tool.callSelectClipImageBlock([self clipImage], self.model.phAsset);
    }
}
